...
```bash
gh run view <RUN_ID> --repo peviitor-scrapers/e-infra-sa-python-scraper --json status -q .status
```
...
(`gh issue list --repo peviitor-scrapers/e-infra-sa-python-scraper --state open`),
